...
export const google = async (req, res, next) => {
    try {
        ...
        if (user) {
            ...
            const name = req.body.name ? req.body.name.split(' ').join('').toLowerCase() : 'user';
            ...
            const newUser = new User({username: name + Math.random().toString(36).slice(-4), email: req.body.email, password: hashedPassword, avatar: req.body.photo});
            await newUser.save();
